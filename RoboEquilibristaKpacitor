...
/************************************************************************
 * Setup Inicial com as ConfiguraÃ§Ãµes para i2C.
 ************************************************************************/
...
void setup() {
  ...
  /* Inicializando o Barramento i2c para comunicaÃ§Ã£o com a MPU6050 */
  Wire.begin();
  ...
  i2c_data[2] = 0x00;   /* 0x1B - Configurar o fundo de escala do Gyro Â±250deg/s - Faixa */
  i2c_data[3] = 0x00;   /* 0x1C - Configurar o fundo de escala do AcelerÃ´metro para Â±2g - Faixa */

  /* ConfigiraÃ§Ãµes do i2c*/
  while(i2cWrite(0x19, i2c_data, 4, false));

  /* PLL tenha como referÃªncia o gyro de eixo X, Desabilitando Sleep Mode */
  ...
  /* Tempo de estabilizaÃ§Ã£o do Sensor MPU6050 */
  delay(100);
  ...
  /* 4 - InicializaÃ§Ã£o do Filtro de Kalman XY */
...
void loop() {
  ...
  /* Leitura dos Dados de AceleraÃ§Ã£o e Gyro do sensor MPU6050 */
  while(i2cRead(0x3B, i2c_data, 14));
  ...
  */  
  ...
  /* Convertendo de Rad/Segundo para Graus/Segundo Calculo da Taxa angular baseado no GiroscÃ³pio */
  gyroXangle = gyroX / 131.0; //deg/s
  gyroYangle = gyroY / 131.0; //deg/s

  /* Estimativa de Ã‚ngulo nos Eixos X e Y usando Filtro de Kalman */
  KalAngleX = KalmanX.getAngle(roll, gyroXangle, dt);
  KalAngleY = KalmanY.getAngle(pitch, gyroYangle, dt);

  /* Mensagens de Debug para verificaÃ§Ã£o dos resultados obtidos com Filtro de Kalman e Calculos dos Angulos com os AcelerÃ´metros */
  Serial.print(KalAngleY); Serial.print("\n"); //Angulo estimado com o filtro de Kalman
  Serial.print(pitch); Serial.print("\t"); //Angulo Calculado com os dados de aceleraÃ§Ã£o da MPU6050
